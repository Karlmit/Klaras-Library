...
// nativeKoboResources is the full resource table a Kobo expects from
// /v1/initialization.
//
// Returning a subset is not a smaller answer -- it is a destructive one. The
// device REPLACES its stored [OneStoreServices] block with whatever it
// receives, so answering with only the image URLs blanked roughly 130
// endpoints on Klara's reader (account_page, autocomplete, categories,
// eula_page, the lot) and sync failed from then on, with nothing on the server
// to show for it.
//
// Every entry therefore points at Kobo's own hosts, exactly as the device would
// have had them, and only the three image keys are overridden to point here.
// Mirrors calibre-web's NATIVE_KOBO_RESOURCES.
func nativeKoboResources() map[string]any {
	return map[string]any{
		...
		"blackstone_header":                                map[string]any{"key": "x-amz-request-payer", "value": "requester"},
		...
		"free_books_page": map[string]any{
